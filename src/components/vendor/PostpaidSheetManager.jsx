...
const ASSIGNED_PREVIEW_COUNT = 4;
...
/* ── Assigned Value Sheets Modal ───────────────────────────── */
const AssignedSheetsModal = ({ sheets, onClose }) => {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-900 dark:text-white leading-tight">
                Sheets With Assigned Value
              </h3>
              <p className="text-[10px] text-gray-400 mt-0.5">
                {sheets.length} sheets · values can be updated anytime · INR{" "}
                {formatAmount(totalBudget)} total budget
              </p>
            </div>
          </div>
          <button
...
const PostpaidSheetManager = React.memo(
  ({ campaign, token, loadCampaigns }) => {
    ...
    const previewAssigned = assignedSheets.slice(0, ASSIGNED_PREVIEW_COUNT);
    const hasMoreAssigned = assignedSheets.length > ASSIGNED_PREVIEW_COUNT;
          ...
          {assignedSheets.length > 0 && (
            <div className="pt-4 border-t border-gray-100 dark:border-zinc-800">
              ...
              {hasMoreAssigned && (
                ...
                >
                  <Eye className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" />
                  View all {assignedSheets.length} assigned sheets
                </button>
              )}
            </div>
          )}
