...
const COLORS = ["#059669", "#f43f5e", "#fbbf24", "#64748b"];
import {
...
const QR_BASE_URL = (import.meta.env.VITE_QR_BASE_URL || (typeof window !== "undefined" ? window.location.origin : "")).replace(/\/$/, "");
...
const NavButton = ({ active, onClick, icon: Icon, label, badge }) => (
    ...
    >
        ...
        {badge && (
            <span className={`relative z-10 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${active ? "bg-white/20 text-white" : "bg-rose-500 text-white"
                }`}>
                {badge}
            </span>
        )}
...
const VendorAccountManager = ({
    ...
    const handleDownloadCampaignPdf = async (campaign) => {
        console.log("Starting PDF Download for campaign:", campaign?.id);
        if (isPreparingBatchPdf) {
            console.log("Already preparing PDF, ignoring click.");
            return;
        }
        if (!campaign?.id) {
        ...
        try {
            ...
            if (!items.length) {
                console.warn("No QRs found for campaign.");
            ...
            // Wait for React to render the canvases
            await new Promise((resolve) => setTimeout(resolve, 2000));
            console.log("Render timeout complete. Generating PDF...");
            ...
            items.forEach((qr, index) => {
                ...
                const canvas = document.getElementById(getBatchCanvasId(qr.uniqueHash));
                if (!canvas) {
                    // console.warn(`Canvas not found for hash: ${qr.uniqueHash}`);
                    skipped += 1;
                    return;
                }
            ...
            console.log(`PDF Generation done. Skipped: ${skipped}`);
